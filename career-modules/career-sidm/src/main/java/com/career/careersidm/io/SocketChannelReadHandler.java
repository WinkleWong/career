...
public class SocketChannelReadHandler implements CompletionHandler<Integer, StringBuffer> {
	...
	public void completed(Integer result, StringBuffer historyContext) {
		...
		System.out.println("completed(Integer result, Void attachment): 取出通道中准备好的值");
		/**
		...
		if (historyContext.indexOf("over") == -1) {
			...
			//回发数据并关闭channel
			ByteBuffer sendBuffer = ByteBuffer.wrap(URLEncoder.encode("", StandardCharsets.UTF_8.name()).getBytes());
